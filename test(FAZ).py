...
def main(argv=None):
    ...
    for kk,cube in enumerate(cubelist):
        ...
        for num in range(1,20):
            nx = int(np.random.normal(200, 50))
            ny = int(np.random.normal(200, 50))
            if nx<=50 or nx>=350:
                nx=200
            if ny<=50 or ny>=350:
                ny=200
            test_images[0, 0:BLOCK_SIZE[0], 0:BLOCK_SIZE[1], 0:BLOCK_SIZE[2], :] = cube_images[0, :,(nx-50):(nx+50),(ny-50):(ny+50), :]
            score, result0, piece_loss, sf0 = sess.run([y_, pred_, model_loss, sf],
                                                       feed_dict={x: test_images, y: test_annotations})
            result[(nx-50):(nx+50),(ny-50):(ny+50)] =result[(nx-50):(nx+50),(ny-50):(ny+50)]/2+ sf0[0, 0, :,:, 1] * 255/2
        end = time.time()
        ...
        timeuse = end - start
        print("Saved image: ", cube, timeuse)
        loss_sum += loss2
        acc_sum += acc
        dice_sum += dice
